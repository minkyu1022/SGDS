...
def langevin_dynamics(x, log_reward, device, args, expl_model=None):
    accepted_samples = []
    accepted_logr = []
    acceptance_rate_lst = []
    ...
    for i in range(args.max_iter_ls):
        ...
        log_r = log_reward(x, count=True)
        if expl_model is not None:
            log_r = log_r + 100.0 * expl_model(x)
        ...
        if expl_model is not None:
            log_r_new = log_r_new + 100.0 * expl_model(new_x)
