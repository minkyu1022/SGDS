...
parser.add_argument('--first_epochs', type=int, default=25000)
parser.add_argument('--second_epochs', type=int, default=30000)
...
def eval_step(eval_data, energy, gfn_model, final_eval=False):
    ...
    if final_eval:
        metrics['final_eval/energy_call_count'] = energy.energy_call_count
    else:
        metrics['eval/energy_call_count'] = energy.energy_call_count
    ...
    if eval_data is None:
        ...
        if final_eval:
            ...
            metrics['final_eval/EUBO_minus_ELBO'] = metrics['final_eval/EUBO'] - metrics['final_eval/ELBO']
        else:
            ...
            metrics['eval/EUBO_minus_ELBO'] = metrics['eval/EUBO'] - metrics['eval/ELBO']
    ...
    if args.both_ways:
        if it % 2 == 0:
            if args.sampling == 'buffer':
                loss, states, _, _, log_r  = fwd_train_step(energy, rnd_model, gfn_model, exploration_std, return_exp=True)
...
def fwd_train_step(energy, rnd_model, gfn_model, exploration_std, return_exp=False):
    init_state = torch.zeros(args.batch_size, energy.data_ndim).to(device)
    loss = get_gfn_forward_loss(args.mode_fwd, init_state, rnd_model, gfn_model, energy.log_reward, coeff_matrix,
...
def train(name, energy, buffer, buffer_ls, epoch_offset, epochs, log_Z_est=None, pre_gfn=None):
    ...
    return gfn_model, rnd_model, epoch_offset+epochs
...
def teacher_sampling(mode_teacher, buffer, energy, expl_model=None):
    ...
    if mode_teacher == 'ais':
        ...
        samples, rewards = langevin_dynamics(population, energy.log_reward, device, args, expl_model=expl_model)
        buffer.add(samples.detach().cpu(), rewards.detach().cpu())

        log_Z_est = torch.logsumexp(rewards, dim=-1) - torch.log(torch.tensor(rewards.shape[0]))
    ...
    if args.teacher is not None:
        ...
        for i in range(args.round):
            ...
            if i == 0:
                ...
                print(f"After {args.teacher}, total energy calls: {energy.energy_call_count}")
                
                student_model, rnd_model, global_epochs = train(name, energy, buffer, buffer_ls, epoch_offset=global_epochs, epochs=args.first_epochs, log_Z_est=teacher_flow)
                ...
                if args.student_mode == 'reinit':
                    student_model, rnd_model, global_epochs = train(name, energy, buffer, buffer_ls, epoch_offset=global_epochs, epochs=args.second_epochs, log_Z_est=student_model.flow_model, pre_gfn=None)
                    print(f"After TB, total energy calls: {energy.energy_call_count}")
                    ...
                    student_model, rnd_model, global_epochs = train(name, energy, buffer, buffer_ls, epoch_offset=global_epochs, epochs=args.second_epochs, log_Z_est=student_model.flow_model, pre_gfn=student_model)
                    print(f"After TB, total energy calls: {energy.energy_call_count}")
        ...
        student_model, rnd_model, global_epochs = train(name, energy, buffer, buffer_ls, epoch_offset=global_epochs, epochs=args.first_epochs, log_Z_est=None)
